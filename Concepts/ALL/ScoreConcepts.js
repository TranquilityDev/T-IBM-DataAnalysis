...
PFName  = ['曾子耘', '陳盛通', '何艷宏', '洪國仁', '黃定遠', '黃瓊如', '賴炎卿', '吳朝欽', '謝耀智', '許慧雯', '楊建成', '游慧光'];
COOPName  = ['陳聖賢', '翟本瑞', '郭賢', '江宜芳', '李桂秋', '林瑞發', '劉文榮', '汪浩', '吳肇展', '楊坤鋒', '于躍門'];
...
MCAEName  = ['蔡忠杓', '成銘德', '戴國政', '鄧錦坤', '洪榮洲', '黃錦煌', '黃宗立', '林浩庭', '林孟儒', '林源堂', '劉明山', '劉顯光', '羅致卿', '彭信舒', '王啟昌', '蕭肇殷', '楊榮顯', '楊世宏', '尹清岩', '張文政', '朱智義'];
FCMName  = ['蔡昆', '蔡宜壽', '陳文正', '陳致光', '黃建霖', '黃金鳳', '李維平', '廖盛焜', '林佳弘', '邱長塤', '石天威', '鄭國彬'];
...
LMName  = ['陳建元', '何彥陞', '洪本善', '李瑞陽', '林宗賢', '王珍玲', '謝靜琪', '辛年豐', '楊賀雯', '楊龍士', '楊文燦', '張梅英', '朱南玉', '鍾懿萍','周天穎'];
CLName  = ['陳兆南', '何寄澎', '李寶玲', '李時銘', '李玲', '李威熊', '梁煌儀', '廖美玉', '林韻柔', '彭妙卿 ', '宋建華', '王祥齡', '楊美美', '余美玲', '張瑞芬', '鄭慧如', '鐘文伶', '朱文光'];
FLLName  = ['蔡明秀', '蔡依仁', '曾鹿鳴', '沈薇薇', '郭毓芳', '何文敬', '黃舒屏', '李麗秋', '廖咸浩', '林羨', '劉森堯', '劉顯親', '羅珮瑄', '彭芳美', '邱源貴', '薛孟仁', '周玉楨', '莊坤良'];
RMIName  = ['陳森松', '洪介偉', '黃依潔', '江朝', '江耕南', '康裕民', '賴志仁', '寶宏', '吳瑞雲', '許文彥', '張吉宏','黃雅文', '林進田', '劉純之', '呂瑞秋', '張智凱'];
...
EESName  = ['陳建隆', '陳志成', '馮秋霞', '胡苔莉', '黃東池', '簡正一', '李書安', '梁正中', '林俊德', '林秋裕', '任南琪', '童翔新', '王唯匡', '吳俊哲', '吳志超', '喻家駿', '張立德', 'T.-Nejat Veziroglu'];
...
PFCS = [曾子耘CS, 陳盛通CS, 何艷宏CS, 洪國仁CS, 黃定遠CS, 黃瓊如CS, 賴炎卿CS, 吳朝欽CS, 謝耀智CS, 許慧雯CS, 楊建成CS, 游慧光CS ];
COOPCS = [陳聖賢CS, 翟本瑞CS, 郭賢CS, 江宜芳CS, 李桂秋CS, 林瑞發CS, 劉文榮CS, 汪浩CS, 吳肇展CS, 楊坤鋒CS, 于躍門CS ];
STATCS = [陳婉淑CS, 高秀蘭CS, 李燊銘CS, 林文欽CS, 林哲彥CS, 王聖CS, 王婉倫CS, 吳進家CS, 吳榮彬CS, 鍾冬川CS, 賴崑榮CS, 李文CS, 林麗芬CS ];
ECOCS = [陳瓊怜CS, 陳善瑜CS, 陳依依CS, 陳至還CS, 方文碩CS, 郭祐誠CS, 胡士文CS, 賴景昌CS, 李文傳CS, 劉宗欣CS, 歐嘉瑞CS, 彭德昭CS, 秦國軒CS, 王平CS, 王葳CS, 吳紀瑩CS, 徐繼達CS, 顏厚棟CS, 趙志鉅CS, 鄭保村CS, 鄒繼礎CS ];
...
MCAECS = [蔡忠杓CS, 成銘德CS, 戴國政CS, 鄧錦坤CS, 洪榮洲CS, 黃錦煌CS, 黃宗立CS, 林浩庭CS, 林孟儒CS, 林源堂CS, 劉明山CS, 劉顯光CS, 羅致卿CS, 彭信舒CS, 王啟昌CS, 蕭肇殷CS, 楊榮顯CS, 楊世宏CS, 尹清岩CS, 張文政CS, 朱智義CS ];
FCMCS = [蔡昆CS, 蔡宜壽CS, 陳文正CS, 陳致光CS, 黃建霖CS, 黃金鳳CS, 李維平CS, 廖盛焜CS, 林佳弘CS, 邱長塤CS, 石天威CS, 鄭國彬CS ];
...
ARCHCS = [曾亮 CS, 沈揚庭 CS, 陳上元 CS, 崔征國 CS, 杜方中 CS, 高敬賢 CS, 谷特瑪蒂亞斯 CS, 黃德生 CS, 黎淑婷 CS, 李英弘 CS, 林昌明 CS, 林富淞 CS, 林季芸 CS, 林衍良 CS, 彭志峰 CS, 宋玉真 CS, 謝佩霓 CS, 張莉欣 CS, 趙又嬋CS, 劉為光CS, 鄭明仁CS ];
...
LMCS = [陳建元CS, 何彥陞CS, 洪本善CS, 李瑞陽CS, 林宗賢CS, 王珍玲CS, 謝靜琪CS, 辛年豐CS, 楊賀雯CS, 楊龍士CS, 楊文燦CS, 張梅英CS, 朱南玉CS, 鍾懿萍CS,周天穎CS ];
CLCS = [陳兆南CS, 何寄澎CS, 李寶玲CS, 李時銘CS, 李玲CS, 李威熊CS, 梁煌儀CS, 廖美玉CS, 林韻柔CS, 彭妙卿 CS, 宋建華CS, 王祥齡CS, 楊美美CS, 余美玲CS, 張瑞芬CS, 鄭慧如CS, 鐘文伶CS, 朱文光CS ];
FLLCS = [蔡明秀CS, 蔡依仁CS, 曾鹿鳴CS, 沈薇薇CS, 郭毓芳CS, 何文敬CS, 黃舒屏CS, 李麗秋CS, 廖咸浩CS, 林羨CS, 劉森堯CS, 劉顯親CS, 羅珮瑄CS, 彭芳美CS, 邱源貴CS, 薛孟仁CS, 周玉楨CS, 莊坤良CS ];
RMICS = [陳森松CS, 洪介偉CS, 黃依潔CS, 江朝CS, 江耕南CS, 康裕民CS, 賴志仁CS, 寶宏CS, 吳瑞雲CS, 許文彥CS, 張吉宏CS,黃雅文CS, 林進田CS, 劉純之CS, 呂瑞秋CS, 張智凱CS ];
...
MSE= [蔡健益CS, 陳錦山CS, 陳錦毅CS, 何主亮CS, 河合良信CS, 洪緯璿CS, 金重勳CS, 柯澤豪CS, 李立鼎CS, 梁辰睿CS, 林昆明CS, 駱榮富CS, 邱國峰CS, 孫道中CS, 田修波CS, 王誠佑CS, 王致傑CS, 張育誠CS, 張志宇CS, YosiYShachamCS ];
